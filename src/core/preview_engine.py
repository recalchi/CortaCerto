...
class PreviewEngine:
    def __init__(
        ...
        self._requests: queue.Queue[tuple[int, PreviewSettings]] = queue.Queue()
    ...
    def request_frame(self, frame_index: int, settings: PreviewSettings) -> None:
        ...
        if cached is not None:
            ...
            return
        self._requests.put((frame_index, settings))
    ...
    def _run(self) -> None:
        while not self._stop.is_set():
            ...
            try:
                frame_index, settings = self._requests.get(timeout=0.2)
                while True:
                    frame_index, settings = self._requests.get_nowait()
            except queue.Empty:
                continue
